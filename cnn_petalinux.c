/* CNN MNIST Inference Application for Kria KV260 - Q8.8 Fixed-Point */
...
extern const int16_t weights_fixed[];
extern const int16_t test_images_fixed[NUM_TEST_IMAGES][INPUT_SIZE];
extern const uint8_t test_labels[NUM_TEST_IMAGES];

// Convert float to Q8.8 fixed-point
int16_t float_to_fixed(float value)
{
    return (int16_t)(value * 256.0f);
}

// Convert Q8.8 fixed-point to float
float fixed_to_float(uint16_t fixed_value)
{
    int signed_value = (int)fixed_value;
    return (float)signed_value / 256.0f;
}

void load_image_to_bram(const int16_t *image_fixed)
{
...
int run_inference(const int16_t *image_fixed)
{
    setup_cnn_pointers();
...
int main()
{
    ...
    printf("\n========================================\n");
    printf("  CNN MNIST Inference on KV260 (Q8.8)\n");
    printf("========================================\n\n");
